...
@section('content')
    ...
    <div class="page-container">
        <div class="col-12 d-flex justify-content-between align-items-center my-3">
            <div class="col-md-8">
                <h4 class="header-title">{{__('lang.airline_edit')}} {{$receipt->name}}</h4>
            </div>
            <div class="col-md-4">
         ...
         <div class="col-lg-12"> @if (session('success'))
            ...
            <form action="{{route('admin.receipt.update' ,['type' => $type , 'id' => $receipt->id     ])}}" class="card" method="post">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-md-4">
                            ...
                            <input type="text" disabled  value="{{ $receipt->code }}"
                                   class="form-control"/>
                        </div>
                        <div class="col-12 col-md-4">
                            <label for="inpuName3" class="col-form-label">{{__('lang.name')}}</label>
                            <input type="text" name="name" value="{{ $receipt->name }}"
                                   class="form-control"/> @error('name') <span
                                class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 col-md-4">
                            <label for="symbol" class=" col-form-label">{{__('lang.symbol')}}</label>
                            <input type="text" name="symbol"  id="symbol" value="{{ $receipt->symbol }}" class="form-control"/>
                        </div>
                    </div>
                 </div>
