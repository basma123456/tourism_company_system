...
@section('content')
    ...
    <div class="page-container">
        <div class="col-12 d-flex justify-content-between align-items-center my-3">
            ...
            <div class="col-md-4">
                <a   @if(request()->get('list') === 'invoices')  href="{{ url(route('admin.invoices.index')) }}" @else  href="{{ url(route('admin.flight_tickets.index')) }}"  @endif
        ...
        <div class="col-lg-12"> @if (session('success'))
            ...
            <form action="{{route('admin.invoices.update'  , $invoice[0]->inv_id) . '?list=invoices'}}" class="card" method="post">
                @csrf
                @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="container gray_container_invoice">
                            <div class=" row bottom_border">
                                <label for="amounts"
                                       class="col-md-3 col-sm-6 text-primary">{{__('lang.amount_before_tax')}}</label>
                            ...
                            <div class="row bottom_border">
                                <label for="amount"
                                       class=" col-md-3  col-sm-6 text-primary">{{__('lang.amount')}}</label>
                                <h4 id="amount"
                                    class="col-md-9 col-sm-6">{{ $invoice[0]->invoice_amount +  $invoice[0]->invoice_tax  }}</h4>
                            </div>
                                ...
                                <table class="table table-bordered">
                                    ...
                                    @forelse($invoice as $key => $item)
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="number" name="invoice_item_id[]"
                                                       value="{{$item->invoice_item_id}}" class="d-none"> <textarea
                                                    class="textAreaInvoices" name="notes[]">{{$item->notes}}</textarea>
                                            </td>
                                            {{--                                            <td>--}}
                                            {{--                                                @if($item->invoice_id)--}}
                ...
                <div class="col-12 col-md-3 offset-md-9 d-flex justify-content-center justify-content-md-end mb-3 mt-3">
                    <button type="submit" class="btn btn-primary rounded-pill px-3">
                        ...
                        @else
                            <i class="ri-user-add-line"></i> &nbsp;&nbsp; {{__('lang.save')}}
                        @endif
                    </button>
                </div>
            </form>
